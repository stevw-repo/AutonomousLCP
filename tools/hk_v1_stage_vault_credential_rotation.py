...
def _receipt(
    *,
    ...
    credential_names = cast("list[JsonValue]", sorted(_APPLICATION_CREDENTIALS))
    body: dict[str, JsonValue] = {
        "candidate_binding_ref": _binding(candidate),
        "candidate_root": str(candidate_root),
        "credential_names": credential_names,
        "predecessor_binding_ref": _binding(predecessor),
        "rotated_credential_count": len(_APPLICATION_CREDENTIALS),
        "rotation_id": rotation_id,
        "schema_id": "asklegal.hk-v1-vault-credential-staging-receipt",
        "schema_version": "1.0.0",
        "source_root": str(source_root),
        "unchanged_credential_count": len(candidate) - len(_APPLICATION_CREDENTIALS),
    }
...
def stage_vault_credential_rotation(
    *,
    source_root: Path,
    candidate_root: Path,
    receipt_path: Path,
    ...
    """Create or replay one exact full staging set with rotated vault secrets."""
    ...
    if candidate_root.exists():
        ...
        if not generated_secrets.isdisjoint({value[1] for value in old_credentials.values()}):
            _fail("VAULT_CREDENTIAL_GENERATION_FAILED")
    ...
    old_secrets = {value[1] for value in old_credentials.values()}
    ...
    if (
        ...
        or any(
            ...
            if name not in _APPLICATION_CREDENTIALS
        )
    ):
