...
def _codes(
    ...
    return {
        finding.code
        for finding in validate_host_identity_policy(policy, topology or _topology())
    }


def test_host_identity_inputs_are_complete_unallocated_and_uncreated() -> None:
    """Bind every accepted writable-service account without mutating the host."""
