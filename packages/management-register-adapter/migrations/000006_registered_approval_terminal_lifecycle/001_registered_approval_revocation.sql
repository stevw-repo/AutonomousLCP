CREATE PROCEDURE review.revoke_registered_approval_v1
...
BEGIN
    ...
    IF HASHBYTES('SHA2_256', @command_bytes) <> @command_fingerprint
        THROW 51600, 'ASKLEGAL_COMMAND_BYTES_FINGERPRINT_MISMATCH', 1;
    IF HASHBYTES('SHA2_256', @event_bytes) <> @event_fingerprint
        THROW 51601, 'ASKLEGAL_EVENT_BYTES_FINGERPRINT_MISMATCH', 1;
    IF @approval_id NOT LIKE 'apr[_]%' OR @proposal_package_id NOT LIKE 'ppk[_]%' OR
       ...
       @manifest_fingerprint NOT LIKE 'sha256:%' OR LEN(@manifest_fingerprint) <> 71
        THROW 51602, 'ASKLEGAL_APPROVAL_REVOCATION_IDENTITY_INVALID', 1;
    IF ISJSON(@command_json) <> 1 OR
       ISNULL(JSON_VALUE(@command_json, '$.action'), '') <> 'REVOKE_REGISTERED_APPROVAL' OR
       ...
       LEN(ISNULL(JSON_VALUE(@command_json, '$.reason'), '')) NOT BETWEEN 1 AND 2000 OR
       LEN(ISNULL(JSON_VALUE(@command_json, '$.revoker_subject'), '')) NOT BETWEEN 1 AND 255
        THROW 51603, 'ASKLEGAL_APPROVAL_REVOCATION_COMMAND_INVALID', 1;
    IF ISJSON(@event_json) <> 1 OR
       ...
       ISNULL(JSON_VALUE(@event_json, '$.to_state'), '') <> 'APPROVAL_REVOKED' OR
       ISNULL(JSON_VALUE(@event_json, '$.event_type'), '') <> 'REVOKE' OR
       ...
       ISNULL(JSON_VALUE(@event_json, '$.immutable'), '') <> 'true'
        THROW 51604, 'ASKLEGAL_APPROVAL_REVOCATION_EVENT_INVALID', 1;

    IF EXISTS
    (
        SELECT 1 FROM register.command_v1_fact
        WHERE owning_application = 'REVIEW_APPLICATION' AND command_id = @command_id
    )
    BEGIN
    ...
    BEGIN TRY
        EXEC @lock_result = sys.sp_getapplock
            ...
            @LockTimeout = 5000;
        IF @lock_result < 0 THROW 51605, 'ASKLEGAL_APPROVAL_LOCK_FAILED', 1;
        ...
        (
            ...
            THROW 51606, 'ASKLEGAL_APPROVAL_NOT_REVOCABLE', 1;
...
GRANT EXECUTE ON OBJECT::review.revoke_registered_approval_v1
